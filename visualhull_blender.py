...
def paint_voxel(o, d, img, voxel_c, voxel_t, voxel_s, rsize, vsize, t_n, t_f):
    ...
    # not update voxel_c at voxel_ti == 0 (voxel_s[ray_p] is all 0)
    voxel_ti = jnp.where(voxel_ti == 0, jnp.zeros([vsize, vsize, vsize]).astype(jnp.int16) + (vsize+1), voxel_ti)
...
# denoising sphere
def get_sphere(vsize, with_color=False, margin=5):
    if not with_color:
...
def visualhull(FLAGS, dataset, test_dataset=None, target="", dilation=5, thresh=100):
    ...
    if FLAGS.vh_save == "color" or FLAGS.vh_test:
        ...
        voxel_c = voxel_c * get_sphere(vsize, with_color=True)
        ...
        for i in range(N):
            ...
            d = test_dataset.rays.directions[i*10]
            frame = render_voxel(voxel_s, voxel_c, o, d, rsize, vsize, t_n, t_f)
            plt.subplot(6,N,i+1+N*0); plt.imshow(frame)

        for i in range(N):
            frame = test_dataset.images[i*10,Ellipsis,:3]
            plt.subplot(6,N,i+1+N*1); plt.imshow(frame)

        voxel_c_red = jnp.zeros([vsize, vsize, vsize, 3]).astype(jnp.float32) + jnp.array([1.,0.,0.]) * voxel_s[:,:,:,None]
        pred_masks = []
        for i in range(N):
            o = test_dataset.rays.origins[i*10]
            d = test_dataset.rays.directions[i*10]
        ...
        for i in range(N):
            if FLAGS.alpha_bkgd:
                mask = (test_dataset.images[i*10,Ellipsis,3:] > 0).astype(np.float32)
            else:
                mask = np.sum(test_dataset.images[i*10,Ellipsis,:3], axis=2) != 3
...
def main(unused_argv):
    ...
    if FLAGS.alpha_bkgd:
      ...
      assert FLAGS.thresh < 100, "thresh < 100 is recommended"
